...
[RedisViewer3下载](https://www.guangyapan.com/s/1893305391470878805_aeYeUSbIXUmlUXkR)
...
支持结构化搜索：自动分析key拆分结构化，redis库里都存了什么key一目了然（补偿海量数据的搜索方案）
...
下载Redis Viewer-xxx安装包，运行就可以安装Redis Viewer。
    ...
    Linux --> Redis Viewer--linux-2.4.2.flatpak
        通用.flatpak:  依赖flatpak环境
